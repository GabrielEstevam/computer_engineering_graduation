   // Nome da classe:  Fatoracao
   // Escrito por: Gabriel Estevam Oliveira
   // Data: 28 de outubro de 2016
   // Proposito: encontrar os primos que compoem a chave publica.
   //
   // Funcoes:
   //		numero v[] fatoracao(numero N)
   //			- Objetivo: encontrar os numeros primos que sao a fatoracao de um numero N.
   //			- Parametro: numero a ser descoberto a fatoracao.
   //			- Retorno: os dois numero que compoem a fatoracao.
   //			O metodo utilizado para achar os fatores consiste um numero N por um numero A, se o resultado B, multiplicado por A for igual a N entao A e B sao os numeros primos.
   //			O numero A é inicialmente escolhido como o menor numero impar com a metade dos algarismos de N e é acrescentado em dois a cada tentativa. 
   //
...
public class Fatoracao {
    ...
    public static void main(String[] args) throws IOException {
		...
		// Leitura do arquivo
        String text = "/XXXXX/esquemaRSA/chave2.txt";
        String l1 = "";
        int i = 0;
        
        try {
            FileReader arq = new FileReader(text);
        ...
        numero chave = new numero();
        chave.tamanho = l1.length();
        for (i = 0; i < chave.tamanho; i++) {
            chave.n[chave.tamanho - i - 1] = Integer.parseInt(""+l1.charAt(i));
        }
        chave.imprimirNumero();

        numero v [] = fatoracao(chave);
        v[0].imprimirNumero();
        v[1].imprimirNumero();

		// Grava no arquivo
        FileWriter arq = new FileWriter("/XXXXX/esquemaRSA/primos.txt");
        PrintWriter gravarArq = new PrintWriter(arq);
        
        for (i = v[0].tamanho -1 ; i >= 0; i--) {
            gravarArq.printf(v[0].n[i]+"");
        }
        gravarArq.printf("\n");

		for (i = v[1].tamanho -1 ; i >= 0; i--) {
            gravarArq.printf(v[1].n[i]+"");
        }
        gravarArq.printf("\n");
        arq.close();
        System.out.printf("A mensagem foi gravada com sucesso em: /XXXXX/esquemaRSA/primos.txt.\n");
    }
    
	static numero [] fatoracao(numero N) {
        numero v[] = new numero[2];
        numero a = new numero();
        numero b = new numero();
        ...
        while (true){
            if (a.n[0] == 5)
                a = soma(a, dois);
            b = divisao(N, a);
            if (subtracao(multiplicacao(a, b), N).tamanho == 0) {
                break;
            } else {
                a = soma(a, dois);
            }
